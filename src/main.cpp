...
void setup_wifi_and_ntp() {
  ...
  WiFiManagerParameter custom_timezone("timezone", "Timezone (e.g., CET-1CEST,M3.5.0,M10.5.0/3)", "CET-1CEST,M3.5.0,M10.5.0/3", 40);
...
void update_display() {
    ...
    char darian_buf[50];
    sprintf(darian_buf, "%d %s %d", darian_date.day, darian_date.month_name, darian_date.year);
...
void setup() {
    ...
    lv_obj_set_style_bg_color(scr, lv_color_black(), 0); // Placeholder for retrofuturism background image
